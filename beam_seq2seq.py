...
DECAY_COEF = 30
DECAY_NORM = math.sqrt(DECAY_COEF)
...
def _extract_mmiargmax_and_embed(embedding,
  ...
  def loop_function(prev, i, mmi_symbols):
    if output_projection is not None:
      prev = nn_ops.xw_plus_b(prev, output_projection[0], output_projection[1])

    if i < GAMMA:
      lambd = LAMBDA * math.sqrt(DECAY_COEF-i) / DECAY_NORM
      log_probts = tf.nn.log_softmax(prev)      # p(T|S)
      log_probts_sub = tf.subtract(log_probts, tf.scalar_mul(lambd, log_prior))   # p(T|S) - λ.p(T)
      prev_symbol = math_ops.argmax(log_probts_sub, 1)
    else:
      prev_symbol = math_ops.argmax(prev, 1)

    mmi_symbols.append(prev_symbol)
...
def mmi_attention_decoder(decoder_inputs,
                      initial_state,
                      attention_states,
                      cell,
                      output_size=None,
  ...
  with variable_scope.variable_scope(
    ...
    for i, inp in enumerate(decoder_inputs):
      ...
      if loop_function is not None and prev is not None:
        with variable_scope.variable_scope("loop_function", reuse=True):
          inp = loop_function(prev, i, mmi_symbols)
...
def embedding_attention_seq2seq(encoder_inputs,
  ...
  with variable_scope.variable_scope(
    ...
    def decoder(feed_previous_bool):
      ...
      with variable_scope.variable_scope(
          variable_scope.get_variable_scope(), reuse=reuse) as scope:
        outputs, state, mmi_symbols = embedding_attention_decoder(
            decoder_inputs,
            encoder_state,
            attention_states,
            cell,
    ...
    outputs_and_state, mmi_symbols = control_flow_ops.cond(feed_previous,
                                              lambda: decoder(True),
                                              lambda: decoder(False))
    outputs_len = len(decoder_inputs)  # Outputs length same as decoder inputs.
    state_list = outputs_and_state[outputs_len:]
    ...
    if nest.is_sequence(encoder_state):
      state = nest.pack_sequence_as(
          structure=encoder_state, flat_sequence=state_list)
    return outputs_and_state[:outputs_len], state, mmi_symbols
  ...
  with ops.op_scope(all_inputs, name, "model_with_buckets"):
    for j, bucket in enumerate(buckets):
      with variable_scope.variable_scope(variable_scope.get_variable_scope(),
        ...
        bucket_outputs, _, mmi_symbol = seq2seq(encoder_inputs[:bucket[0]],
                                    decoder_inputs[:bucket[1]])
        outputs.append(bucket_outputs)
        mmi_symbols.append(mmi_symbol)
  #print "End**********"

  return outputs, mmi_symbols
